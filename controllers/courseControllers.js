...
class courseController { 
    ...
    static createCourse = async (req,res)=> { 
        try {
            ...
            await CourseModel.create({name,email,phone,dob,address,gender,education,course,user_id:id})
    ...
    static async courseDisplay(req,res) {

        try{ 

            const{name,image,id}=req.udata 
            const course= await CourseModel.find({user_id:id})
            res.render("course/display",{n:name,i:image,c:course,msg:req.flash("success")}) 
...
static async  courseUpdate(req,res)
     {
      try{ 
